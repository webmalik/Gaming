...
function serviceAnimation() {
  ...
  if (window.innerWidth < 992) {
    tl.fromTo(
      ".service1",
      { y: "0", rotate: 0 },
      { y: "0%", rotate: 0, duration: 20 }
    ); // Змінено тривалість на 2 секунди
    tl.fromTo(
      ".service2",
      { y: "100%", rotate: 30 },
      { y: "-108%", rotate: 0, duration: 20 }
    ); // Змінено тривалість на 2 секунди
    tl.fromTo(
      ".service3",
      { y: "200%", rotate: 60 },
      { y: "-170%", rotate: 0, duration: 20 }
    ); // Змінено тривалість на 2 секунди
  } else {
    tl.fromTo(
      ".service1",
    ...
    tl.fromTo(
      ".service2",
      { y: "100%", rotate: 30 },
      { y: "-102%", rotate: 0, duration: 20 }
    ); // Змінено тривалість на 2 секунди
    tl.fromTo(
      ".service3",
      { y: "200%", rotate: 60 },
      { y: "-178%", rotate: 0, duration: 20 }
    ); // Змінено тривалість на 2 секунди
  }
...
function animateProducts() {
  ...
  if (section) {
    ...
    let windowHeight = window.innerHeight,
      ...
      firstPosterRect = posters[0].getBoundingClientRect(),
      centerOffset =
        windowWidth / 2 - (firstPosterRect.left + firstPosterRect.width / 2),
      postersCount = posters.length,
      randomIndex = Math.floor(Math.random() * postersCount),
      rotationFunction = function (index) {
        var value = -3.75 * index + Math.random();
    ...
    gsap
      .timeline({
        scrollTrigger: {
          ...
          onEnter: () => {
            posters.forEach(
              (poster) => (poster.style.willChange = "transform")
            );
          },
        },
      })
...
function processAnimation() {
  ...
  if (window.innerWidth < 991) {
    ...
    // gsap.set(processList, { yPercent: 20 });
  ...
  if (firstTitleRow) {
    ...
    gsap
      ...
      .staggerFrom(
        ...
        0.05
      ) // Поява першого рядка тексту
      .addLabel("start") // Додаємо мітку "start" для подальшої використання
      .to(
        [".pin-spacer"],
        0.4,
        { backgroundColor: "#6A16AB", ease: "power3.out" },
        "start"
      ...
      .to(splitSecondTitle.chars, 0.4, {
        opacity: 0,
        ease: "power3.out",
        duration: 1,
      }) // Зникнення другого рядка тексту
      .staggerFrom(
